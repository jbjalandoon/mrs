...
class PermissionsModel extends \CodeIgniter\Model
{
    ...
    protected $allowedFields = ['name_on_class', 'function_name', 'function_description','slugs','page_title','module_id','allowed_appointments','status', 'created_at','updated_at', 'deleted_at'];
    ...
    public function update_permitted_role($role_id, $function_id, $prev_function_id = 0)
	{
		...
		{
			$str = str_replace(']', ','.$role_id.']', $function['allowed_appointments']);
			$val_array['allowed_appointments'] = $str;
			return $this->update($function_id, $val_array);
		}

		if($prev_function_id != 0 && $prev_function_id == $function_id)
		{
			$val_array['allowed_appointments'] = $function['allowed_appointments'];
			return $this->update($function_id, $val_array);
		}
		/*
		if($prev_function_id == 0 && in_array($id, json_decode($function['allowed_appointments'])))
		{
			$val_array['allowed_appointments'] = $function['allowed_appointments'];
		...
		if($prev_function_id != 0 && $prev_function_id != $function_id)
		{
			...
			$str = str_replace(','.$role_id, '', $function_prev['allowed_appointments']);
			$val_array_prev['allowed_appointments'] = $str;
